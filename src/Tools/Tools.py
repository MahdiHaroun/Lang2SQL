...
# -----------------------------
# Initialize ChatGroq and NLP generator
# -----------------------------
try:
...
# -----------------------------
# Import production-ready Redis client
# -----------------------------
import sys
sys.path.append(os.path.join(os.path.dirname(__file__), "../"))
from src.redis_client import redis_client

# Simple session connector cache to prevent connection leaks
session_connectors = {}
...
def update_db_connector(connection_string: str, session_id: str = "default"):
    """
    Create/update a session in Redis and store the connection string.
    Returns success status and stores session data in Redis for persistence.
    """
    try:
        # Save connection string and metadata in Redis with expiration
        session_data = {
        ...
        # Create and cache database schema for faster access
        try:
